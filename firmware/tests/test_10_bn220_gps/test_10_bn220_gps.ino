 ...
 *
 * Wiring (BN-220):
 *   BN-220 VCC (red)    → ESP32 3.3V         (DO NOT use 5V on bench
 *                                              unless you know your
 *                                              module's TX is regulated
 *                                              to 3.3V — most are, but
 *                                              3.3V is unconditionally
 *                                              safe.)
 *   BN-220 GND (black)  → ESP32 GND
 *   BN-220 TX  (yellow) → ESP32 GPIO 17  (UART2 RX in this sketch)
 *   BN-220 RX  (white)  → ESP32 GPIO 18  (UART2 TX, optional —
 *                                          leave disconnected unless
 *                                          you plan to send config
 *                                          commands to the GPS)
 *
...
const uint8_t  GPS_TX_PIN = 18;   // ESP32 writes to this (→ BN-220 RX)
